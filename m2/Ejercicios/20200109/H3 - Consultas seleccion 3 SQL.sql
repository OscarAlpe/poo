...
SELECT edad FROM ciclista WHERE nomequipo = 'Banesto' OR nomequipo = 'Navigare';
...
SELECT LEFT(nomequipo, 1) FROM ciclista WHERE LEFT(nombre, 1) = 'R';
...
SELECT numetapa FROM puerto GROUP BY numetapa HAVING ROUND(AVG(altura), 2) > 1500;

-- 1.16 Indicar el número de etapas que cumplen la condición anterior
SELECT COUNT(*) FROM (SELECT numetapa FROM puerto GROUP BY numetapa HAVING ROUND(AVG(altura), 2) > 1500) miselect;

-- 1.17 Listar el dorsal del ciclista con el número de veces que ha llevado algún maillot
SELECT dorsal, COUNT(codigo) FROM lleva GROUP BY dorsal;

-- 1.18 Listar el dorsal del ciclista con el código de maillot y cuantas veces ese ciclista ha llevado ese maillot
SELECT dorsal, codigo, COUNT(codigo) FROM lleva GROUP BY dorsal, codigo;
